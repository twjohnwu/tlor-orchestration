...
containing `.bak` (stale/backup copies). As of this writing that's thirteen
...
`cirdan-shipwright`, `bombadil-freeagent`) — a fourteenth role added later is
picked up automatically, no script edit needed.
...
at least one zero-dispatch role (thirteen roles rarely all fire in the same
