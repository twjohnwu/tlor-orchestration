...
- **輕量**：只裝 plugin。安裝後，任何一個新開的 session 都能使用十四個角色
  （若是在已開啟的 session 中安裝，須先執行 `/reload-plugins`）。請直接
  以名稱明確呼叫角色，或加上 [roles.md](roles.md) 的 CLAUDE.md snippet 來
  穩定派工。我們的 headless 測試顯示，僅靠 description 並不能穩定觸發自動
  派工，所以 snippet 是輕量路線的建議做法。
- **完整**：再加跑 `/tlor-init`。這會落地 rules 檔案、`~/.claude/institution/`
  layout（見下）以及 CLAUDE.md/AGENTS.md 路由。Rules 檔案一旦存在就會自行
  載入，`.claude/rules/` 是原生 auto-load 位置，不需要路由。路由給的是另外
  三件事：最先讀到的派工紀律提醒、給不讀 `.claude/rules/` 的工具用的
  AGENTS.md 介面，以及宣告本框架的角色是你的主要派工對象。
...
- **`rules/customize/` 是你的。** installer 會建立這個目錄，首次安裝可能
  幫你種入選配的起始檔案，之後**永遠不覆蓋**裡面已存在的任何東西。持久的
  本地客製化內容只該放在這裡。
- **Base 檔案沒有任何使用者可寫區塊。** 使用者自行新增的內容（lessons、
  skill namespace 優先序表、本地慣例）一律放在 `rules/customize/`，
  絕不放進 base rule 檔案：那裡的任何追加內容都會在下次無條件覆蓋時被清空。
...
裝好之後，rules corpus 載不載入不是由 routing 決定：大部分內容會在**每個
session 開始時整包載入**。`~/.claude/rules/`（連同它的 `customize/` 子目錄）
是 Claude Code 原生的 auto-load 位置，底下每個沒有 `paths:` frontmatter 的
`.md` 檔都會在啟動時遞迴載入進 context，不需要 `@import`。帶有 `paths:`
frontmatter 的 rule 檔案則是官方支援的延後載入方式，它不在啟動時載入，
只在 Claude 讀到符合該 pattern 的檔案時才載入。

用 `wc -l -c` 對這個 repo 實測，自我量測指令見下方。這只是某一次的快照，
不是保證值，請自行重跑：
...
下限是 **1,145 行／約 60.0 KB**，還不含你之後自己加的任何一條 lesson。
這是每個 session 都要付的固定稅，不管那個 session 有沒有派出任何 subagent。
base 數字適用於每一種安裝方式；合計數字只在你同時裝了選配的
`rules/customize/` 種子檔（`install.sh --with-optional`）時才成立，只裝
base 的安裝就只付 base 那個數字。

這個機制有兩個誠實提醒。**版本下限**：`.claude/rules/` 的 auto-load 需要
Claude Code 2.0.64 以上，舊版根本不讀這個目錄，所以「不需要路由」在舊版上
的結果是一條 rule 都沒載入，而不是你以為的輕量 fallback。**它是可以被關掉
的**：任何 settings 層的 `claudeMdExcludes` 都能抑制載入；另外，當
`--setting-sources` 排除了 project settings 時，project 層的 rules 也會被
跳過，這一半只適用於 project 層 rules，與使用者層安裝無關。別假設這包
corpus 是無條件生效的。

這個成本對 context window 較小的模型影響會成比例地更大，而那正是這個
framework 的目標讀者：派工這整套機制的前提，就是把 field work 從有限的
context 卸載出去。如果你在意每個 session 的 budget，把這一點跟上面輕量的
...
hook。在既有或 `--continue`/`--resume` 的 session 中執行 `--install-hook`
不會讓 hook 在那個 session 生效，請只在全新 session 中驗證。
...
且不變的。明確宣告 `--skills-dest`，就是你選擇跳出這個預設，替刻意放在
別處的 skills 目錄開一道後門。
...
無論哪種方式，裝完**都要開新 session**，因為 agent 定義是在 session 啟動時
載入的。
