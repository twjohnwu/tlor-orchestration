...
("general-purpose" or "claude") or a built-in Explore/Plan type ("explore" or
"plan") unless the prompt carries the literal marker "[bombadil-freeagent]" AND
an explicit `model` parameter is passed. This is the L2 backstop for dispatch.md
§3: naming slips that bypass the pinned role table get redirected instead of
...
def main():
    try:
        ...
        if subagent_type not in GENERIC_SUBAGENT_TYPES | BUILTIN_SHADOWED_TYPES:
            return 0
        ...
        # Guarded dispatch with no opt-out marker/model → deny
        if subagent_type in BUILTIN_SHADOWED_TYPES:
            deny_reason = (
                ...
                "the task needs tools/MCP permissions no tlor role has, add "
                "[bombadil-freeagent] to the prompt AND pass an explicit model."
            )
        else:
            deny_reason = (
